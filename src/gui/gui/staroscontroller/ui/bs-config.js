  ...
  proxy: {
    ...
    middleware: function (req, res, next) {
      ...
      if (!!viewName && !isExternalApp && defaultViews.indexOf(viewName[1]) === -1) {
        ...
        req.url = req.url.replace('/staroscontroller/ui/', '/apps/' + viewName[1] + '/app/src/main/resources/');
        proxy.web(req, res);
      }
      else if (isExternalApp) {
        // in this case it is an external application (not in ONOS source code) that extend the view
        // so we redirect the request to the app folder in case of .js, .css, .html
        req.url = req.url.replace('/staroscontroller/ui/', '/src/main/resources/');
        proxy.web(req, res);
      }
      // NOTE onos.js and index.html should not be proxied (require server side injection)
      else if (req.url.match(/(?:js|css|html)/) && req.url !== '/staroscontroller/ui/onos.js' && 
               req.url !== '/staroscontroller/ui/index.html' && req.url !== '/staroscontroller/ui/nav.html') {
        // redirect onos base js files to the source folder
        req.url = req.url.replace('/staroscontroller/ui/', '/web/gui/src/main/webapp/');
        proxy.web(req, res);
      }
      else {
        return next();
