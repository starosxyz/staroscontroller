...
describe('factory: fw/remote/urlfn.js', function () {
    ...
    it('should return the correct (http) RS url', function () {
        setLoc('http', 'foo', '123');
        expect(ufs.rsUrl('path')).toEqual('http://foo:123/staroscontroller/ui/rs/path');
    });

    it('should return the correct (https) RS url', function () {
        setLoc('https', 'foo', '123');
        expect(ufs.rsUrl('path')).toEqual('https://foo:123/staroscontroller/ui/rs/path');
    });

    it('should return the correct (ws) WS url', function () {
        setLoc('http', 'foo', '123');
        expect(ufs.wsUrl('path')).toEqual('ws://foo:123/staroscontroller/ui/websock/path');
    });

    it('should return the correct (wss) WS url', function () {
        setLoc('https', 'foo', '123');
        expect(ufs.wsUrl('path')).toEqual('wss://foo:123/staroscontroller/ui/websock/path');
    });

    it('should allow us to define an alternate WS port', function () {
        setLoc('http', 'foo', '123');
        expect(ufs.wsUrl('xyyzy', 456)).toEqual('ws://foo:456/staroscontroller/ui/websock/xyyzy');
    });

    it('should allow us to define an alternate host', function () {
        setLoc('http', 'foo', '123');
        expect(ufs.wsUrl('core', 456, 'bar')).toEqual('ws://bar:456/staroscontroller/ui/websock/core');
    });

    it('should allow us to inject an app context', function () {
        setLoc('http', 'foo', '123', '/my/app');
        expect(ufs.wsUrl('path')).toEqual('ws://foo:123/my/app/staroscontroller/ui/websock/path');
    });
});
