...
/*
 ONOS GUI -- Component Policy View Module
 */
...
(function () {
    ...
    // constants
    var pName = 'policy-details-panel',
        panelWidth = 540,
        topPdg = 60,
		
		propOrder = ['policyid', 'srcIp', 'dstIp','content'],//JSON中的key
		friendlyProps = [
			'POLICYID', 'SRCIP', 'DSTIP','CONTENT'
		];
    ...
    function populateTop(details) {
        ...
        var propsBody = top.select('.policy-props').append('tbody');

        top.select('.policy-title-1').text('策略管理');
        top.select('.policy-title-2').text(details.policyid);
    ...
    angular.module('ovPolicy', [])
        ...
        function (_$log_, _$scope_, _wss_, _fs_, _ks_, _ps_, _is_, tbs) {
            ...
            ks.gestureNotes([
                ['click row', 'Select / deselect policy property'],
                ['scroll down', 'See more policy']
            ]);
            ...
            $log.log('OvPolicyCtrl has been created');
        }])

        .directive('policyDetailsPanel',
